...
def cluster_themes(keyword_freq: dict) -> list[dict]:
    """
    keyword_freq: {keyword: count}
    Returns: [{"theme_name": str, "keywords": [...], "total_frequency": int}, ...]
    """
    if not keyword_freq:
        return []

    # 빈도순 정렬, 상위 50개만 LLM에 넘김
    top_keywords = sorted(keyword_freq.items(), key=lambda x: -x[1])[:50]
    keyword_list = "\n".join(f"- {kw} ({cnt}회)" for kw, cnt in top_keywords)

    prompt = f"""다음은 오늘 아침 수집된 경제·증시 뉴스의 주요 키워드와 빈도입니다.
이를 의미적으로 유사한 테마끼리 묶어주세요.
...
{{
  "themes": [
    {{
      ...
      "description": "이 테마가 오늘 증시에 미치는 영향 1~2문장"
    }}
  ]
}}"""
    ...
    for attempt in range(config.LLM_MAX_RETRIES):
        try:
            ...
            logger.info("테마 %d개 클러스터링 완료", len(themes))
            return themes
        except Exception as e:
            logger.warning("테마 클러스터링 오류 (시도 %d/%d): %s", attempt + 1, config.LLM_MAX_RETRIES, e)
            if attempt < config.LLM_MAX_RETRIES - 1:
                time.sleep(config.LLM_RETRY_DELAY_SEC * (attempt + 1))

    logger.error("테마 클러스터링 최종 실패")
    return []
...
if __name__ == "__main__":
    ...
    sample_freq = {
        "연준": 8, "금리": 6, "FOMC": 4,
        "엔비디아": 5, "AI": 4, "반도체": 3, "TSMC": 2,
        "삼성": 3, "실적": 3,
    }
    themes = cluster_themes(sample_freq)
    print("\n--- 테마 클러스터링 결과 ---")
    for t in themes:
