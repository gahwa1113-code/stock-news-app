"""
Bloomberg 뉴스 크롤러 (RSS 피드)
실행 테스트: python -m scrapers.bloomberg
"""
import calendar
import logging
import time
...
def fetch_bloomberg_news() -> list[dict]:
    """Bloomberg RSS 피드에서 뉴스 수집"""
    all_articles = []

    for rss_url in config.BLOOMBERG_RSS_URLS:
        try:
            ...
            logger.warning("Bloomberg RSS 수집 오류 (%s): %s", rss_url, e)
    ...
    logger.info("Bloomberg: %d개 기사 수집", len(unique))
    return unique


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(message)s")
    results = fetch_bloomberg_news()
